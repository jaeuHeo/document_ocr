...
def update_correct(request):
    ...
    try:
        ...
        if conn:
            conn.rollback()
        return responseCode(resultCode=500, resultData='다른 관리자가 템플릿을 삭제하여 데이터베이스 조회 불가', resultMsg=e)
    finally:
        conn.close()
...
def delete_image(request):
    ...
    try:
        ...
        if conn:
            conn.rollback()
        return responseCode(resultCode=500,resultData='삭제 에러',resultMsg=e)
    finally:
        if conn:
            conn.close()
...
def clf(request):
    ...
    try:
        ...
        for idx,rs in enumerate(result_df):
            del result_df[idx][-1]
