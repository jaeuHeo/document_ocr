...
#         # conn = psycopg2.connect(host='172.16.114.128', dbname='doc_classifier', user='dba', password='DBPdbA123!@#')
#         # cur = conn.cursor()
#         return conn
...
def post_request(img_path, end_point='text_detection', add_data={}):
    try:
        ...
        response = requests.request("POST", verify=False, url=settings.MODEL_API_URL + end_point + '/', files=myfile,
                                    data=add_data)
...
class api_model():
    ...
    def ocr_to_merge(self, ocr_data, img):
        ...
        for i in ocr_data.get('data', []):
            # if float(i['per']) >= 0.5:
            pts = i['vertices']
            result_list['bboxes'].append(
                [pts['x1'], pts['y1'], pts['x2'], pts['y2'], pts['x3'], pts['y3'], pts['x4'], pts['y4']])
    ...
    def template_key_far(self):
        ...
        while idx < len(info_list):
            ...
            if int(idx) != int(len(info_list) - 1):
                template_far.append([(next_info['p1'][0] + ((next_info['p2'][0] - next_info['p1'][0]) / 2) - info['p1'][
                    0] + ((info['p2'][0] - info['p1'][0]) / 2)) / self.regist_sh[1],
                                     (next_info['p1'][1] + ((next_info['p4'][1] - next_info['p1'][1]) / 2) - info['p1'][
                                         1] + ((info['p4'][1] - info['p1'][1]) / 2)) / self.regist_sh[0],
                                     info_list[idx][2]])
            else:
    ...
    def find_samekey(self, results, key_count):
        ...
        # test_kbox_list = []
        bbox_info_cp = [
            [info['sr_value'], info['sr_keyword'], info['keyword']] if info['sr_value']['p1'] != [0, 0] and
                                                                       info['sr_value']['p3'] != [0, 0] else [
                info['sr_keyword'], info['sr_value'], info['keyword']] for idx, info in
            enumerate(self.bbox_info)].copy()
        # bbox_info_cp[0] = [bbox_info_cp[0][1],bbox_info_cp[0][0],bbox_info_cp[0][2]]

        # info_list = []
        # if self.endpoint == 'classify':
        #     info_list = [ [sr_k, sr_v, k] for sr_k,sr_v,k in zip(self.bbox_info['sr_keyword'],self.bbox_info['sr_value'],self.bbox_info['keyword']) if sr_v['p1'] !=[0, 0] and sr_v['p3'] !=[0, 0]]

        # elif self.endpoint == 'document_form':
        # info_list = self.form_info.copy()

        for idx1 in range(start, len(bbox_info_cp)):

            info = bbox_info_cp[idx1]

            if info[0]['p1'] != [0, 0] and info[0]['p3'] != [0, 0]:
                ...
                for idx2, k in enumerate(results[0].values.tolist()):
                    ...
                    if acc_cer2 > 0.9:
                        ...
                        if idx1 != 0:
                            append_list = [[text, self.bbox_info[idx1]['sr_keyword'], self.bbox_info[idx1]['sr_value']],
                                           [text2, test_kbox]]

                            arr_ocr.append(append_list)

                        # if test_kbox not in list(map(lambda x:x[0],test_kbox_list)):
                        # test_kbox_list.append([test_kbox,text])
                        ...
                        # bbox_dic = {}
                        # bbox_dic['key_box'] = self.bbox_info[idx1]['sr_keyword']
                        resz_cen_x, resz_cen_y = 0, 0
                        if idx1 == 0:
                            key_box = bbox_info_cp[idx1][0]
                            resz_cen_x = (key_box['p1'][0] + (key_box['p2'][0] - key_box['p1'][0]) / 2) / \
                                         self.regist_sh[1]
                            resz_cen_y = (key_box['p1'][1] + (key_box['p4'][1] - key_box['p1'][1]) / 2) / \
                                         self.regist_sh[0]
                        ...
                        diff_cen_xy = [(test_list[0] - resz_cen_x) * self.regist_sh[1],
                                       (test_list[1] - resz_cen_y) * self.regist_sh[0]]

                        if key_count > 4:
                            for ids, (cr_val, cr_key, keyword) in enumerate(bbox_info_cp):
                                if idx1 == 0:

                                    bbox_info_cp[ids][0] = {'p1': [cr_val['p1'][0] + diff_cen_xy[0],
                                                                   cr_val['p1'][1] + diff_cen_xy[1]],
                                                            'p2': [cr_val['p2'][0] + diff_cen_xy[0],
                                                                   cr_val['p2'][1] + diff_cen_xy[1]],
                                                            'p3': [cr_val['p3'][0] + diff_cen_xy[0],
                                                                   cr_val['p3'][1] + diff_cen_xy[1]],
                                                            'p4': [cr_val['p4'][0] + diff_cen_xy[0],
                                                                   cr_val['p4'][1] + diff_cen_xy[1]]}
        ...
        # for idx, k in enumerate(results[0].values.tolist()):
        #
        #     text2 = ''.join([q for q in k[-3]])
        #
        #     text2 = convert_to_text(text2)
        #
        #     if char_error_cnt(doc_name, text2, return_type='per') > 0.9:
        #
        #         if key_count < 4:
        #
        #             test_list = [
        ...
        #             bbox_dic = {}
        #             # if self.endpoint == 'classify':
        #             #     bbox_dic['val_box'] = self.bbox_info['sr_value'][0]
        #
        #             # elif self.endpoint == 'document_form':
        #             bbox_dic['val_box'] = self.bbox_info[0]['sr_value']
        #
        #             resz_cen_x = (bbox_dic['val_box']['p1'][0] + (bbox_dic['val_box']['p2'][0] - bbox_dic['val_box']['p1'][0]) / 2) / self.regist_sh[1]
        #             resz_cen_y = (bbox_dic['val_box']['p1'][1] + (bbox_dic['val_box']['p4'][1] - bbox_dic['val_box']['p1'][1]) / 2) / self.regist_sh[0]
        #
        #             test_cen_x, test_cen_y = test_list[0], test_list[1]
        #
        #             test_kbox_list = [(test_cen_x - resz_cen_x) * self.test_sh[1], (test_cen_y - resz_cen_y) * self.test_sh[0]]
        #
        #         else:
        #
        #             test_kbox_list.append([k[0:8],doc_name])
        #             break
        #
        # test_kbox_list.sort(key=lambda x: x[0][7])
    ...
    def model(self, test_img_cp, results, num_test=0):
        ...
        for regist, test in arr_ocr:
            ...
            point_rec_box = [
                [(test[1][0] / self.test_sh[1] + (regist[2]['p1'][0] - regist[1]['p1'][0]) / self.regist_sh[1]) *
                 self.test_sh[1] - (regist_h / 2 * self.test_sh[0]),
                 (test[1][1] / self.test_sh[0] + (regist[2]['p1'][1] - regist[1]['p1'][1]) / self.regist_sh[
                     0] - regist_h / 4) * self.test_sh[0]],

                [(test[1][2] / self.test_sh[1] + (regist[2]['p2'][0] - regist[1]['p2'][0]) / self.regist_sh[1]) *
                 self.test_sh[1] + (regist_h / 2 * self.test_sh[0]),
                 (test[1][3] / self.test_sh[0] + (regist[2]['p2'][1] - regist[1]['p2'][1]) / self.regist_sh[
                     0] - regist_h / 4) * self.test_sh[0]],

                [(test[1][4] / self.test_sh[1] + (regist[2]['p3'][0] - regist[1]['p3'][0]) / self.regist_sh[1]) *
                 self.test_sh[1] + (regist_h / 2 * self.test_sh[0]),
                 (test[1][5] / self.test_sh[0] + (regist[2]['p3'][1] - regist[1]['p3'][1]) / self.regist_sh[
                     0] + regist_h / 4) * self.test_sh[0]],

                [(test[1][6] / self.test_sh[1] + (regist[2]['p4'][0] - regist[1]['p4'][0]) / self.regist_sh[1]) *
                 self.test_sh[1] - (regist_h / 2 * self.test_sh[0]),
                 (test[1][7] / self.test_sh[0] + (regist[2]['p4'][1] - regist[1]['p4'][1]) / self.regist_sh[
                     0] + regist_h / 4) * self.test_sh[0]]
            ]

            point_rec_box_list.append(sum(point_rec_box, []))

            pts_xy = np.array([test[1][0], test[1][1], test[1][2], test[1][3], test[1][4], test[1][5], test[1][6],
                               test[1][7]]).reshape(-1, 2).astype(np.int32)

            pts = np.array(point_rec_box).reshape(-1, 2).astype(np.int32)
            test_img_cp = cv2.polylines(test_img_cp, [pts_xy], True, (0, 255, 0), 5)
            test_img_cp = cv2.polylines(test_img_cp, [pts], True, (0, 0, 255), 5)
        ...
        for ids, info in enumerate(self.bbox_info):
                ...
                if keytext == lab:
                    ...
                    if len(pts) > 0:
                        if self.endpoint == 'classify':
                            ...
                            same_label.append(
                                [value_dic['value_text'][idx].split(' ')[0], pts[0][0], pts[0][1], pts[1][0], pts[1][1],
            ...
            pts_xy = [minmax_xy[0], minmax_xy[1], minmax_xy[2], minmax_xy[1], minmax_xy[2], minmax_xy[3], minmax_xy[0],
                      minmax_xy[3]]
            ...
            if self.endpoint == 'classify':
                ...
                result_df.append(
                    [int(num_test), ids, keytext, ' '.join([txt for txt in same_df['text'].values]), minmax_xy])
                ...
                result_df.append([keytext,
                                  ...
                                  [{'p1': x[1:3], 'p2': x[3:5], 'p3': x[5:7], 'p4': x[7:9]} for x in
                                   same_df.values.tolist()]])
        ...
        for idx1, i in enumerate(transform['detection']):
            ...
            same_df = pd.DataFrame(columns=['text', 'x1', 'x4', 'y1', 'y4'])
            same_df = same_label_merge(same_label, same_df)
            r_df = [transform['label'][idx1], ' '.join(same_df['text'])]
            if self.endpoint == 'classify':

                if len(result_df[idx1][3]) == 0 and len(r_df[1]) > 0:
                    result_df[idx1][3] = r_df[1]
                    pts_df = pd.DataFrame(pts_df, columns=['x1', 'y1', 'x3', 'y3'])
                    minmax_xy = [pts_df['x1'].min(), pts_df['y1'].min(), pts_df['x3'].max(), pts_df['y3'].max()]
                    pts_xy = [minmax_xy[0], minmax_xy[1], minmax_xy[2], minmax_xy[1], minmax_xy[2], minmax_xy[3],
                              minmax_xy[0], minmax_xy[3]]
                ...
                if len(result_df[idx1][1]) == 0 and len(r_df[1]) > 0:
                    result_df[idx1][1] = r_df[1]
        ...
        if key_count > 4:
            ...
            while idx < len(test_kbox_list):
                ...
                if int(idx) != int(len(test_kbox_list) - 1):
                    test_far.append([(next_info[0] + ((next_info[2] - next_info[0]) / 2) - info[0] + (
                                (info[2] - info[0]) / 2)) / self.test_sh[1],
                                     (next_info[1] + ((next_info[7] - next_info[1]) / 2) - info[1] + (
                                                 (info[7] - info[1]) / 2)) / self.test_sh[0],
            ...
            bbox_info_cp = [
                [info['sr_value'], info['sr_keyword'], info['keyword']] if info['sr_value']['p1'] != [0, 0] and
                                                                           info['sr_value']['p3'] != [0, 0] else [
                    info['sr_keyword'], info['sr_value'], info['keyword']] for info in self.bbox_info].copy()
            ...
            for temp_x, temp_y, text1 in template_far:
                ...
                if text1 in list(map(lambda x: x[2], test_far)):
                    ...
                    diff_x, diff_y = (test_far[test_far_idx][0] - temp_x) * self.regist_sh[1], (
                                test_far[test_far_idx][1] - temp_y) * self.regist_sh[0]
            ...
            for info in self.bbox_info:
                ...
                if val['p1'] == [0, 0] and val['p3'] == [0, 0]:
                    ...
                    trans.append(
                        [(key['p1'][0] * self.resize_sh[1]) + test_kbox_list[0],
                         (key['p1'][1] * self.resize_sh[0]) + test_kbox_list[1],
                         (key['p2'][0] * self.resize_sh[1]) + test_kbox_list[0],
                         (key['p2'][1] * self.resize_sh[0]) + test_kbox_list[1],
                         (key['p3'][0] * self.resize_sh[1]) + test_kbox_list[0],
                         (key['p3'][1] * self.resize_sh[0]) + test_kbox_list[1],
                         (key['p4'][0] * self.resize_sh[1]) + test_kbox_list[0],
                         (key['p4'][1] * self.resize_sh[0]) + test_kbox_list[1]])

                else:
                    trans.append(
                        [(val['p1'][0] * self.resize_sh[1]) + test_kbox_list[0],
                         (val['p1'][1] * self.resize_sh[0]) + test_kbox_list[1],
                         (val['p2'][0] * self.resize_sh[1]) + test_kbox_list[0],
                         (val['p2'][1] * self.resize_sh[0]) + test_kbox_list[1],
                         (val['p3'][0] * self.resize_sh[1]) + test_kbox_list[0],
                         (val['p3'][1] * self.resize_sh[0]) + test_kbox_list[1],
                         (val['p4'][0] * self.resize_sh[1]) + test_kbox_list[0],
                         (val['p4'][1] * self.resize_sh[0]) + test_kbox_list[1]])
    ...
    def make_insert_info(self, result_df, num_doc, test_img_cp):
        ...
        for idx, (area, data) in enumerate(zip(self.bbox_info, result_df)):
            ...
            # else:
            row = (
            num_doc, idx, str(json.dumps(area['sr_keyword'])), str(json.dumps(area['sr_value'])), area['keyword'],
            data[1], str(json.dumps(data[2])))
            insert_info.append(row)
